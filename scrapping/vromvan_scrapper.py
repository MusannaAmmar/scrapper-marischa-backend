...
class VromvanScrapper:
	def __init__(
		self,
		output_file="json_files/vromvan_jobs.json",
		source_url="https://vroomsearch.com/en/vacatures",
		apikey=os.getenv("ZENROWS"),
	):
		self.output_file = output_file
		self.source_url = source_url
		self.base_url = "https://vroomsearch.com"
		self.apikey = apikey
		self.jobs = []
	...
	def _extract_listing_cards(self, html, page_url):
		...
		cards = soup.select("article.node--type-vacancy.node--view-mode-teaser")
		jobs = []

		for card in cards:
			title_node = card.select_one(".field--name-title")
			labels = card.select(".labels p")
			type_node = card.select_one(".labels-padded .type")
			snippet_node = card.select_one(".body-wrapper")
			link_node = card.select_one(".button-wrapper a")

			title = self._clean_text(title_node.get_text()) if title_node else ""
			location = self._clean_text(labels[0].get_text()) if len(labels) > 0 else ""
			company = self._clean_text(labels[1].get_text()) if len(labels) > 1 else ""
			job_type = self._clean_text(type_node.get_text()) if type_node else ""
			...
			status = "active"
			if "expired" in snippet.lower() or "closed" in snippet.lower():
				status = "expired"
		...
		next_node = soup.select_one("li.pager__item--next a[rel='next']")
		next_url = urljoin(page_url, next_node.get("href")) if next_node and next_node.get("href") else None

		return jobs, next_url

	def parse_job_listings(self):
		print(f"Fetching Vroom jobs from: {self.source_url}")
	...
	def _extract_description_from_detail_html(self, html):
		...
		detail_article = soup.select_one("article.node--type-vacancy.node--view-mode-full")
		if detail_article:
			for sel in [".button-wrapper", ".share-wrapper", "nav", "footer", "header", ".pager", ".job-alert-wrapper"]:
				for tag in detail_article.select(sel):
					tag.decompose()

			head = detail_article.select_one(".head")
			body = detail_article.select_one(".body-wrapper")

			head_text = self._normalize_multiline_text(head.get_text("\n", strip=True)) if head else ""
			body_text = self._normalize_multiline_text(body.get_text("\n", strip=True)) if body else ""

			merged = "\n\n".join([x for x in [head_text, body_text] if x]).strip()
			if len(merged) >= 120:
				return merged

		main_content = soup.select_one(".layout-content")
		if main_content:
			text = self._normalize_multiline_text(main_content.get_text("\n", strip=True))
			return text
	...
	def fetch_job_descriptions(self, delay=0.2):
		jobs_to_update = [
			...
			if str(job.get("source") or "").strip().lower() == "vroom"
			and job.get("link")
			and len(str(job.get("description") or "").strip()) < 120
		]

		if not jobs_to_update:
			print("No Vroom jobs need description updates.")
			return

		print(f"Fetching descriptions for {len(jobs_to_update)} Vroom job(s)...")
		success_count = 0
		failed_count = 0
